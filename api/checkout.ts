...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { items, metadata, shippingFlat } = req.body as {
    items: { id: string; name: string; price: number; quantity: number; image?: string; description?: string }[];
  ...
  try {
    ...
    if (shippingFlat && shippingFlat > 0) {
      lineItems.push({
        price_data: {
          currency: "usd",
          unit_amount: Math.round(shippingFlat * 100),
