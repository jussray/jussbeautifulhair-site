...
requireValue((manifest.cookies ?? []).length === 0, 'the public storefront must not declare first-party cookies');
requireValue(Array.isArray(manifest.allowedCookieWriters) && manifest.allowedCookieWriters.length === 0, 'the public storefront must not declare cookie writers');
...
for (const scanRoot of manifest.scanRoots ?? []) {
  ...
  for (const file of files) {
    ...
    if (writerPatterns.some((pattern) => pattern.test(source))) errors.push(`undeclared cookie writer: ${repoPath}`);
  }
}
...
console.log('Declared first-party cookies: 0');
